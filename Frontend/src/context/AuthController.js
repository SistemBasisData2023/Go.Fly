...
export const login = (setUser, loginData) => {
  return axios
    .post(`${BASE_URL}/account/login`, loginData)
    .then((response) => {
      // Handle successful login
      console.log(response.data);
      const token = response.data.token; // Access the token property from the response
      const { surname } = response.data; // Access user data from the response
      const userData = {
        surname
      };
      setUser(userData);
...
export const register = (setUser, registerData) => {
  return axios
    .post(`${BASE_URL}/account/register`, registerData)
    .then((response) => {
      // Handle successful registration
      console.log(response.data);
      const userData = response.data; // Access user data from the response
      setUser(userData);
      return userData;
    })
    .catch((error) => {
